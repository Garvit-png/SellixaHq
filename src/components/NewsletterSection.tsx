  ...
  return (
    <section className="w-full flex justify-center py-24 bg-[#050505] relative z-20 overflow-hidden border-t border-white/5">
      <div className="max-w-4xl w-full flex flex-col items-center px-4 md:px-8 relative z-10 text-center">
        <h2 className="text-3xl md:text-5xl text-white font-serif font-light leading-tight mb-6">
          Join the <span className="text-[#FACC15] italic">Inner Circle.</span>
        </h2>
        ...
        <form className="w-full max-w-md flex flex-col sm:flex-row gap-3 relative" onSubmit={(e) => e.preventDefault()}>
          <input 
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-full px-6 py-4 text-white placeholder:text-white/30 focus:outline-none focus:border-[#FACC15]/50 transition-colors"
            required
          />
          <button 
            type="submit" 
            className="bg-[#FACC15] text-black font-semibold rounded-full px-8 py-4 hover:bg-[#D4AF37] transition-colors"
          >
            Subscribe
          </button>
        </form>
