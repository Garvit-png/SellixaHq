...
export interface BookCfg {
  ...
  spineFont?: string;
  chapters?: string[];
  /** URL opened when the primary CTA ("Buy Now") is clicked in the detail panel. */
  ctaHref?: string;
  /** Label for the primary CTA button. Defaults to "Buy Now". */
  ctaLabel?: string;
}
    ...
    <div
      ...
      className={cn(
        ...
        uiMode === 'hero'
          ...
          : 'bg-[var(--bs-bg-light)] text-[var(--bs-fg-light)]',
        className,
      )}
      style={themeVars}
    >
      ...
      {showDetailPanel && (
        ...
        >
          <h1 className={`m-0 text-[var(--bs-pink)] text-[clamp(24px,2.8cqw,38px)] font-extrabold leading-[1.1] tracking-[-0.01em] @max-[760px]:text-[clamp(20px,6cqw,28px)] ${dpChild(50)}`}>
            {selectedCfg?.title}
          </h1>
          <p className={`mt-[18px] max-w-[46ch] text-[var(--bs-lav)] text-[clamp(14px,1.1cqw,17px)] leading-[1.6] line-clamp-3 @max-[760px]:mt-3 @max-[760px]:line-clamp-2 @max-[760px]:text-[13px] ${dpChild(130)}`}>
            {selectedCfg?.desc}
          </p>
          <div className={`mt-[28px] flex items-center gap-[12px] @max-[760px]:mt-[14px] ${dpChild(210)}`}>
            <div className="flex gap-[4px]">
              {[0, 1, 2, 3, 4].map((i) => (
                <svg
                  key={i}
                  viewBox="0 0 24 24"
                  className={`h-[20px] w-[20px] fill-[var(--bs-pink)] ${i < (selectedCfg?.stars ?? 0) ? '' : 'opacity-25'}`}
                >
                  <path d="M12 2.6l2.8 6 6.6.6-5 4.4 1.5 6.5L12 16.7 6.1 20.1l1.5-6.5-5-4.4 6.6-.6z" />
                </svg>
              ))}
            </div>
            <span className="text-[14px] text-[var(--bs-lav)]/60">{selectedCfg?.year}</span>
          </div>
          <div className={`mt-[26px] border-t border-[var(--bs-lav)]/[0.18] @max-[760px]:mt-4 ${dpChild(270)}`} />
          <div className={`pointer-events-auto mt-8 ${dpChild(330)}`}>
            {selectedCfg?.ctaHref ? (
              <a href={selectedCfg.ctaHref} target="_blank" rel="noopener noreferrer" className="no-underline">
                <CreepyButton
                  coverClassName="bg-[var(--bs-pink)] text-[var(--bs-navy)] text-[15px]"
                  className="text-[15px]"
                >
                  {selectedCfg.ctaLabel ?? 'Read PDF ↗'}
                </CreepyButton>
              </a>
            ) : null}
          </div>
