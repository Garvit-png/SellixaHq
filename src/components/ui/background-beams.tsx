...
const animations = pathData.map((_, i) => ({
  duration: 12 + (i % 5) * 2, // Slowed down from 4 + 0.8 to 12 + 2
  delay: i * 0.3, // Spread out the start times more
...
export const BackgroundBeams = React.memo(({ className }: BackgroundBeamsProps) => {
  return (
    <div className={cn("pointer-events-none absolute inset-0 h-full w-full", className)}>
      <svg
        aria-hidden="true"
        className="absolute h-full w-full drop-shadow-[0_0_15px_rgba(244,180,0,0.5)]"
        fill="none"
        viewBox="0 0 696 316"
        ...
        <defs>
          {pathData.map((_, i) => (
            <linearGradient
              ...
              y2="100%"
            >
              <stop offset="0%" stopColor="#FFF5CC" stopOpacity="0" />
