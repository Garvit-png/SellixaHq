...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  // Always render null on server — avoids hydration mismatch
  const [mounted, setMounted] = useState(false);
  useEffect(() => { setMounted(true); }, []);

  // SSR-safe mobile check — runs only on client
  const isMobile = useMemo(() => {
    if (typeof window === 'undefined') return true; // hide on SSR
  ...
  useEffect(() => {
    ...
    // Track whether cursor is over the hero video section
    let isOverHero = false;
    const heroEl = () => document.getElementById('intro');

    const updateCursorColor = (color: string) => {
      if (!cursorRef.current || !cornersRef.current || !dotRef.current) return;
      gsap.to(Array.from(cornersRef.current), { borderColor: color, duration: 0.2, ease: 'power2.out' });
      gsap.to(dotRef.current, { backgroundColor: color, duration: 0.2, ease: 'power2.out' });
    };

    // Also keep track of current "zone color" so leave handlers restore correctly
    let zoneColor = cursorColor;
    ...
    const moveHandler = (e: MouseEvent) => {
      ...
      if (hero) {
        ...
        if (inside && !isOverHero) {
          isOverHero = true;
          zoneColor = '#ffffff';
          updateCursorColor('#ffffff');
        } else if (!inside && isOverHero) {
          isOverHero = false;
          zoneColor = cursorColor;
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      const leaveHandler = () => {
        ...
        if (cursorColorOnTarget && cornersRef.current) {
          gsap.to(Array.from(cornersRef.current), { borderColor: zoneColor, duration: 0.15, ease: 'power2.out' });
          if (dotRef.current)
            gsap.to(dotRef.current, { backgroundColor: zoneColor, duration: 0.15, ease: 'power2.out' });
        }
