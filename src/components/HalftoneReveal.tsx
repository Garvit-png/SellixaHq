...
const SHAPES: Record<string, number> = { circle: 0, square: 1, diamond: 2, line: 3 };
const TRIGGERS: Record<string, number> = { off: 0, hover: 1, always: 2 };
...
void main() {
  ...
  if (uMode == 2) {
    ...
    c = clamp(c - gcr, 0.0, 1.0); m = clamp(m - gcr, 0.0, 1.0);
    y = clamp(y - gcr, 0.0, 1.0); k = clamp(max(gcr, k * k * 0.9), 0.0, 1.0);
    ...
    print = uPaper; print = mix(print, ink2, covB * 0.85); print = mix(print, uInk, covA);
  } else {
